...
SELECT 
  ...
  END AS para_ordenar, 
libros.id, libros.codigo, UPPER(libros.nombre) AS nombre, libros.tipo AS tipo_id, tipo.nombre AS tipo, CONCAT('$', FORMAT(libros.donativo, 2)) AS donativo, inventario.piezas, inventario.obsequiados, CONCAT('$', FORMAT(libros.donativo * inventario.piezas,2)) AS importe, CONCAT('$', FORMAT(libros.donativo * inventario.obsequiados,2)) AS importe_obs, inventario.id AS registro, libros.barras FROM libros LEFT JOIN (SELECT * FROM inventario WHERE usuario='".$_SESSION['id']."') AS inventario ON libros.id=inventario.libro INNER JOIN tipo ON libros.tipo=tipo.id ORDER BY $ordenar_por";
...
*/
  ...
  if($guardar){
	    $query = "UPDATE consulta SET ordenar='$ordenar' WHERE usuario='$usuario_id'";
...
<table class="libros" border="1" cellpadding="2" cellspacing="2">
  <tbody>
    <tr class="col_head">
      ...
      <td style="text-align: center;">BARRAS</td>
    </tr>
<?php //PHP
  $lista_obsequiados="";
	$availableTags="";
...
//	        });
        ...
        <tr class='$existencia' id='row_$id'>
          ...
          <td id='tipo_$id'>$tipo</td>
          <td id='donativo_$id'>$donativo</td>
          <td class='centrado' id='mas_menos_$id'>
          ...
          <td><input id='barras_$id' value='$barras'><button id='barras_guardar_$id' class='barras_guardar'>Guardar</button></td>
        </tr>
	    ";
	    }
      if($ordenar=="0"){
...
*/
      ...
      $hay_sin_exist="oculto";
	    if(mysqli_num_rows($result)){
        	while($row = mysqli_fetch_array($result)){
              ...
              $registros_tipo=$registros_tipo."
              <tr class='$existencia' id='row2_$id'>
      ...
      echo "$titulo_tipo $existencia_tipo$titulo_tipo2$hay_sin_exist$titulo_tipo3
$registros_tipo";
      echo "
      <tr class='espacial $existencia_tipo' id='row2_new'>
...
";
      if($ordenar!="0"){
		    $total_imp=money_format("$%!i",$total_imp);
					    echo "
		    <tr id='totales_tipo_per' class='$existencia_tipo'>
			    <td class='derecha' colspan='3'>Total</td>
...
<?php //php
    ...
    $letras_html=$letras_html."<a href='javascript:void(0);' id='nav_per'><div class='$clase'>P*</div></a>";
    ...
    $letras_html="<a href='javascript:void(0);' id='nav_per'><div class='$clase'>PERSONALIZADOS</div></a>".$letras_html;
	...
	$last_edited=filectime('../js/barras.js');
	echo "<script type='text/javascript' src='js/barras.js?$last_edited'></script>";
	if($ordenar=="0"){
